...
# Graduate Research Assistant, Dept of CEE, University of washington
...
input_start=2001-01-10
# Input End Date Here
input_end=2017-08-06
...
param=('SM01_Percentile' 'RadT_tavg' 'Rainf_f_tavg' 'Evap_tavg')
...
paramID=('soilmoisture' 'temperature' 'precipitation' 'evapotranspiration')
...
for((k=0; k<${#region[*]}; k++))
do
# Writing Region Name in a text file
...
# Adding parameter name in the file name to calculate timeseries average of the parameter using python
cp ./Corr_$curDate.asc ./${regionID[k]}_${modelID[i]}_${paramID[j]}_$curDate.asc
...
gdaldem color-relief ./${regionID[k]}_${modelID[i]}_${paramID[j]}_$curDate.tif ${paramID[j]}.txt -alpha ./col_${regionID[k]}_${modelID[i]}_${paramID[j]}_$curDate.tif
...
# Extracting QSB_TAVG and QS_TAVG parameters to calculate timeseries average 
gdal_translate -b 1 -of AAIGrid NETCDF:"FLDAS_${modelnot[i]}_A_${region[k]}_D.A${curDate}.001.nc":Qsb_tavg ./FLIP_$curDate.asc
python fliprasterafrica.py
cp ./Corr_$curDate.asc ./${regionID[k]}_${modelID[i]}_qsbtavg_$curDate.asc

gdal_translate -b 1 -of AAIGrid NETCDF:"FLDAS_${modelnot[i]}_A_${region[k]}_D.A${curDate}.001.nc":Qs_tavg ./FLIP_$curDate.asc
python fliprasterafrica.py
cp ./Corr_$curDate.asc ./${regionID[k]}_${modelID[i]}_qstavg_$curDate.asc

# Running python scripts to calulate timeseries average
python runoff_timeseries.py
python precip_timeseries.py
done
done
rm *.tif *.asc *.xml
